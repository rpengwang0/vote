...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            $post_data = [];
            $post_data['img'] = new CURLFile($this->filePath);
            $result = $this->requestUrl('http://'.$_SERVER['HTTP_HOST'].'/ticket/activity_api/imgUpload',$post_data);
            $result_json = json_decode($result,true);
            if($result_json['code'] == 0){
                $this->stateInfo = $this->getStateInfo("ERROR_UNKNOWN");
                return;
            }
            $this->fullName = $result_json['data']['img_url'];
