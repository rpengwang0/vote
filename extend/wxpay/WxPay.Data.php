...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign($config, $needSignType = true)
	{
		...
		file_put_contents(
		    "make_sign.log",date("Y-m-d H:i:s").PHP_EOL.
            json_encode($this->values).PHP_EOL,FILE_APPEND);
		...
		file_put_contents(
            "make_sign.log",date("Y-m-d H:i:s").PHP_EOL.$string
            .PHP_EOL,FILE_APPEND);
        ...
        file_put_contents(
            "make_sign.log",date("Y-m-d H:i:s").PHP_EOL.$result
            .PHP_EOL,FILE_APPEND);
		return $result;
	}
	/**
	 * 生成签名  第二次签名，调用接口，区分第一次签名，主要做了一次驼峰转化
	 * @param WxPayConfigInterface $config  配置对象
	 * @param bool $needSignType  是否需要补signtype
	 * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
	 */
	public function MakeSign2($config, $needSignType = true)
	{
		if($needSignType) {
			$this->SetSignType($config->GetSignType());
		}
		...
		$string = $this->ToUrlParams();
			...
			//如果是二次签名需要 转大写appId
			$string = str_replace("appid","appId",$string);

		$string = str_replace("noncestr","nonceStr",$string);
$string = str_replace("timestamp","timeStamp",$string);
...
class WxPayAppApiPay  extends WxPayDataBase{
	...
	* @param string $value 
	**/
	public function SetPrepay($value)
	{
