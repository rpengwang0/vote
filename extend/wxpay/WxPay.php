...
class WxPayApi
{
     ...
     * Time: 2019-03-26
     */
    public static function weChatPayChange($wxConfig,$amount,$re_openid,$desc="测试",$check_name=""){
        ...
        $data=array(
            'mch_appid'=>$wxConfig->GetAppId(),//商户账号appid
            'mchid'=> $wxConfig->GetMerchantId(),//商户号
        ...
        //获取签名
        $data['sign'] = self::getSign($data,$wxConfig->GetKey());

        //这是新写的一个curl 请求方法    里面的证书地址是写的绝对路径下的 sdb_app
        $resultXml = self::curlPost($transfers_url,self::arrayToXml($data));

        //这是现有微信支付的请求方式  会报错  证书不对
//        $startTimeStamp = self::getMillisecond();//请求开始时间
//        $resultXml = self::postXmlCurl($wxConfig,self::arrayToXml($data),$transfers_url,false,$startTimeStamp);

        $result = json_decode(json_encode(simplexml_load_string($resultXml, 'SimpleXMLElement', LIBXML_NOCDATA)),true);
    ...
    {
        $isdir = $_SERVER['DOCUMENT_ROOT']."/extend/wxpay/cert/sdb_app";//证书位置;绝对路径
